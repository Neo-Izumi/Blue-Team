...
class role_options(Cog):
    ...
    async def stages(self, ctx): 
        ...
        if roles.value == None:
            ...
            if user.dic_user[author.id].rep == True:
            ...
            if user.dic_user[author.id].cow == True:
                await ctx.send('+) Task no.4: establish SSH connection: \U00002B55')
            else:
                await ctx.send('+) Task no.4: establish SSH connection: \U0000274C')
                
            if user.dic_user[author.id].ca == True and user.dic_user[author.id].ftp == True and user.dic_user[author.id].pen and user.dic_user[author.id].rep == True and user.dic_user[author.id].cow == True:
                await asyncio.sleep(3)
                await ctx.send(embed = embed.embed_final('All tasks are finished'))
                await ctx.send('```Contributor: Nguyễn Doanh Thịnh - ThinhNDHE161890```')
