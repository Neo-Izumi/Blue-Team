...
class roles(View):
    ...
    # async def incident_response(self, button3: Button, interaction: Interaction):
    #     message = '''Ok. Your work has completed'''
    #     await interaction.response.send_message(message, ephemeral=False)
    #     self.value = 'environment'
    #     self.stop()
        
    @nextcord.ui.button(label = 'alert analysis', style = ButtonStyle.red)
    async def alert_analysis(self, button1: Button, interaction: Interaction):
...
Today, you will just need to check the log files, taskes assigned from network manager and email from other department.'''
    ...
    async def incident_response(self, button2: Button, interaction: Interaction):
...
Today, we will give you some useful tools, shortly introduce you guy to these tools and what you can do with the alert at the tier_1 report'''
    ...
    # @nextcord.ui.button(label = 'simulating vulnarable environment', style = ButtonStyle.gray, row=3)
    # async def incident_response(self, button3: Button, interaction: Interaction):
    #     message = '''Ok. Your work has completed'''
    #     await interaction.response.send_message(message, ephemeral=False)
    #     self.value = 'environment'
